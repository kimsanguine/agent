...
이 Part는 하네스 엔지니어링의 **헌법(Constitution)**과 **작업 구조(Task Structure)**를 심화 구현하는 핵심 가이드입니다.

### 연결 지점

- **헌법 + 작업 구조 심화**: YAML Front Matter로 정의하는 타입/우선순위/메트릭이 작업 구조의 메타데이터이며, AI Evals는 검증 기준을 코드화합니다.
- **가장 핵심적인 Harness 구현**: 5축 프레임워크와 폴더 구조 설계가 Repository Harness를 실전에서 구축하는 방법입니다.

**참고**: [dependency-map.md](./dependency-map.md)의 헌법(Constitution) 매핑 섹션을 먼저 읽으면 이 Part의 내용이 더 명확해집니다.

## 왜 CLAUDE.md가 PM의 가장 중요한 파일인가

기본편([3.3-project-memory.md](./3.3-project-memory.md))에서 메모리 계층 구조를 배웠다면, 이제 실전으로 들어간다.
