...
이 Part는 하네스 엔지니어링의 **검증 기준(Validation)**에서 모델 선택을 최적화하는 가이드입니다.

### 연결 지점

- **검증 기준 — 모델 선택 최적화**: "이 작업에는 어떤 모델이 최적인가"를 결정하는 의사결정 트리가 비용-품질 검증 기준입니다.
- **비용 효율성**: 검증 기준의 비용 최적화 관점에서 태스크별 최적 모델을 자동 라우팅합니다.
