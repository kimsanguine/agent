...
이 Part는 하네스 엔지니어링의 **검증 기준(Validation)**에서 도구 선택을 검증하는 가이드입니다.

### 연결 지점

- **검증 기준 — 도구 선택 검증**: "어떤 상황에서 Code vs Cowork를 선택하는가"의 의사결정 기준이 검증 메커니즘입니다.
- **Harness 실행 경로**: 도구 선택에 따라 Repository Harness(Code)와 Application Harness(Cowork)의 적용 방식이 달라집니다.
