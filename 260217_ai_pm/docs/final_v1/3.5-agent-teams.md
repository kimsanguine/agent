...
이 Part는 하네스 엔지니어링의 **실행 루프(Execution Loop)**에서 병렬 실행과 조율을 구현하는 가이드입니다.

### 연결 지점

- **실행 루프 — 병렬 실행 및 조율**: Agent Teams의 Delegate 모드와 태스크 의존성 관리가 Harness의 병렬 실행 메커니즘입니다.
- **Discovery/Definition 적용**: 이 팀 구조는 Part 5(리서치)와 Part 6(PRD 멀티에이전트 리뷰)에서 실전 적용됩니다.
...
**이전 문서**: [3.4-custom-subagents.md](./3.4-custom-subagents.md) — 커스텀 서브에이전트
