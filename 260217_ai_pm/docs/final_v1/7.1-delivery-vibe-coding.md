...
이 Part는 하네스 엔지니어링의 **실행 루프(Execution Loop)**를 통합 개발 세션으로 구현하는 가이드입니다.

### 연결 지점

- **실행 루프 — 통합 개발 세션**: 1M Context로 전체 프로젝트 맥락을 한 세션에서 유지하며, Part 4.1의 MCP + Part 4.2의 CLAUDE.md + Part 3.5의 Agent Teams를 통합 실행합니다.
- **Repository Harness 실현**: "전체 구조 유지" 원칙을 1M Context로 구현 가능하게 한 실전 사례입니다.
